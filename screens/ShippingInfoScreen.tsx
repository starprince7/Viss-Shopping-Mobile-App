...
export default function ShippingInfo() {
  ...
  const params = route.params as { isSettingsView: boolean };
  const isSettingsView = params?.isSettingsView;
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const deliveryInformation = useSelector(selectShippingInfo);
  const [isAddShippingInfoModalOpen, setAddShippingInfoModalOpen] =
    useState(false);
  const [isUpdateShippingInfoModalOpen, setUpdateShippingInfoModalOpen] =
    useState(false);
      ...
      >
        ...
        <ScrollView style={tw`flex-1`} showsVerticalScrollIndicator={false}>
          <View style={tw`p-5 bg-transparent`}>
            ...
            <Text style={tw`font-semibold mt-2 mb-3`}>
              Please select your shipping information below.
            </Text>
          </View>

          {/* ************* Modal-View to `Create or Add` Shipping Information Screen Start ********** */}
          <Modal visible={isAddShippingInfoModalOpen} animationType="slide">
            <View
              lightColor="#eee"
              darkColor="#1B1F22"
          ...
          <Modal visible={isUpdateShippingInfoModalOpen} animationType="slide">
            <View
              lightColor="#eee"
              darkColor="#1B1F22"
              style={tw`bg-[#eee] dark:bg-[#1B1F22] flex-1`}
          ...
          <View style={tw`w-full bg-transparent px-6`}>
            <Button
              onPress={toggleAddShippingInfoModal}
              color={"#89A67E"}
            ...
            {deliveryInformation.length !== 0 ? (
              ...
              <View style={tw`bg-transparent`}>
                <MaterialIcons
                  name="local-shipping"
                  size={160}
                  style={tw`text-neutral-400 mx-auto mt-10 mb-5`}
                />
                <Text style={tw`text-center text-neutral-400 font-semibold`}>
                  You do not have a delivery information, please add your
                  shipping information to continue shopping.
                </Text>
              </View>
            )}
          </View>
        ...
        {!isSettingsView && (
          <TouchableOpacity
            disabled={
              deliveryInformation.length === 0 ||
              selectedShippingInfo._id === null
            }
            onPress={() => navigation.navigate("OrderSummary")}
            style={[
              tw`rounded-[10px] shadow-sm mx-auto mb-2 px-3.5 py-3 flex-row items-center justify-between`,
              deliveryInformation.length !== 0 &&
              selectedShippingInfo._id !== null
                ? tw`bg-[#89A67E]`
                : tw`bg-[#c4d4bf]`,
          ...
          >
            ...
            <HeaderIcon
              name="navigate-next"
              customStyle={tw`text-white mx-auto ml-1.5`}
            />
          </TouchableOpacity>
        )}
      </View>
