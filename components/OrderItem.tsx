...
const OrderItem = ({ image, title, price, quantity }: OrderItemProps) => {
  return (
    <View
      style={tw`flex-row justify-between items-end py-4 px-1 bg-transparent`}
    >
      <View style={tw`flex-row items-center bg-transparent`}>
        <Image style={tw`rounded-3xl h-17 w-16 mr-3`} source={{ uri: image }} />
      ...
      <View style={tw`bg-transparent`}>
        ...
        <Text style={tw`font-semibold`}>
          <Naira style={tw`text-neutral-600 text-xs`}>{price}</Naira>
        </Text>
      </View>
    </View>
  );
