...
export default function Products() {
    ...
    if (isFetchingProduct) {
        return (
            <>
                <Text style={tw`dark:text-white z-10 font-semibold p-5 mx-auto`}>Loading...</Text>
                <View style={tw`bg-transparent flex-row flex-wrap justify-between mx-5`}>
                    {skeletons.map((_, index) => <ProductComponentSkeleton key={index} />)}
                </View>
            </>
        )
    }
