    ...
    <Pressable onPress={selectThisShippingInformation}>
      ...
      >
        {SelectedShippingInfo._id === _id ? (
          <HeaderIcon
            name="check-circle"
            customStyle={tw`text-[#89A67E] text-3xl absolute right-6 top-5`}
          />
        ) : (
          <HeaderIcon
            name="circle"
            customStyle={tw`text-gray-300 dark:text-zinc-700 text-3xl absolute right-6 top-5`}
          />
        )}
        <Text style={tw`mt-2 mb-1 font-semibold`}>Delivery Address</Text>
        <Text style={tw`mb-2 text-zinc-500 dark:text-zinc-300`}>
          {homeAddress}
        </Text>
        ...
        <Button
          color="#89A67E"
          title="Update Info"
          onPress={handleUpdateShippingInformation}
        />
      </View>
    </Pressable>
  );
